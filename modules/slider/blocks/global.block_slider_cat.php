...
if( ! nv_function_exists( 'nv_block_slider' ) )
{
	function nv_block_config_slider( $module, $data_block, $lang_block )
	{
		...
		foreach( $list as $l )
		{
			...
			$html_input .= $xtitle_i . '<label><input type="checkbox" name="config_catid[]" value="' . $l['catid'] . '" ' . ( ( in_array( $l['catid'], $data_block['catid'] ) ) ? ' checked="checked"' : '' ) . '</input>' . $l['title'] . '</label><br />';
		}
		$html .= $html_input;
		$html .= '</td>';
		$html .= '</tr>';
